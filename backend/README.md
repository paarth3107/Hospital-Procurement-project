...
Registration → document upload/verification → categories → dashboard, replacing the single-step "register and you're basically done" flow. Step 1 (drop Category Declaration from registration) and step 2 (document upload) are done; gating the vendor portal on document approval, the staff-side verify/reject UI, and moving category selection to post-approval are still pending.
