...
- **Post-approval category picker** (`frontend/app.js`'s `renderVendorCategoryPicker`): the vendor dashboard now shows a real category checklist once the vendor is Active, sourced from the live catalog (`GET /products`, same pattern as everywhere else). Checking categories and submitting calls the existing (unchanged) `POST /mappings` once per catalog entry in each selected category, using the vendor's own logged-in identity -- no backend changes needed for this, `POST /mappings` already accepted any `vendor_id`. Duplicate requests are reported as "already requested," not shown as failures.
